...
Public Class Nireo_CatEquipos_BIAS_View
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        lstControles = New Controles_Captura().FindlstAll(CInt(Plataforma), CInt(menu + "1"), 0)
    ...
    End Sub
    Sub LlenaDDL()
        Call New CatEquipos().LlenaEquipos(DDLFiltroEquipo, 1)
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New Nireo_CatEquiposBIAS().FindAll(0, 0, 0)
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Function Valida() As Boolean
        ...
        Try
            If gv1.Rows.Cast(Of GridViewRow)().Where(Function(c) _
                                                         TryCast(c.FindControl("TBBIAS"), TextBox).Text = ""
                                                         ).Count > 0 Then
    ...
    Sub Guardar()
        Dim IsResult As Boolean = False
        Try
            If DDLFiltroEquipo.SelectedValue.ToString = "" Then
                mpe_notifications(lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 28).NomMensaje, 3)
                Exit Sub
            ElseIf DDLFiltroCategoriaP.SelectedValue.ToString = "" Then
            ...
            gv1.Rows.Cast(Of GridViewRow)().ToList.ForEach(Sub(r)
                                                               Dim CveEquipo As String = TryCast(r.FindControl("CveEquipo"), Label).Text
                                                               ...
                                                               Dim BIAS As String = TryCast(r.FindControl("BIAS"), Label).Text
                                                               Dim TBBIAS As TextBox = TryCast(r.FindControl("TBBIAS"), TextBox)

                                                               lstValores.Add(CveEquipo + "#" + CveCategoriaP + "#" + CveParametro + "#" + TBBIAS.Text)
                                                           End Sub)

            Dim ObjM As New Nireo_CatEquiposBIAS()
