...
Public Class Nukaxan_UsuariosI_View
    Inherits Page
    ...
    Sub LlenaDDL()
        ...
        Call New Catalogos().LlenaRoles(DDLFiltroRol)
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New Usuario().FindAll(1, 0, "", "I", 0)
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Sub Agregar()
        ...
        Try
            ...
            IsResult = ObjM.SaveModel(0, DDLUsuarioPE.SelectedValue, "I", DDLRolPE.SelectedValue.ToString, ObjUser.CodUsuario)
            If IsResult Then
                FiltrosAcciones(1)
                Dim filtro As String = String.Join("|", (lstFiltros.Select(Function(a) a.Valor).ToList()))
                Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-" + menu + "-1").PaginaURL.Replace("@Id", Codif(DDLUsuarioPE.SelectedValue.ToString)).Replace("@filtro", Codif(filtro)).Replace("@pageIndex", gvindexpage.Text), True)
            Else
    ...
    Sub Eliminar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            Dim ObjM As New Usuario()
            IsResult = ObjM.DeleteModel(CInt(keyId), "I", keyId2, ObjUser.CodUsuario)
            If IsResult Then
                ObtenDatos()
                Alertas("", "17", False, 2)
            Else
