...
Public Class ProdAves_AdminClientes_View
    Inherits Page
    ...
    Sub LlenaDDL()
        ...
        Call New Catalogos().LlenaCtePais(DDLFiltroPais)
        Call New Catalogos().LlenaCteOrigen(DDLFiltroOrigen)
        Call New Catalogos().LlenaClientesAll(DDLClienteE, 2, Plataforma, 1)
        'Call New Catalogos().LlenaUsuarioInterno(DDLUsuarioPE)
        'Call New Catalogos().LlenaRoles(DDLRolPE)
    End Sub
    Sub SeguridadLoad()
    ...
    Protected Sub GVRowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gv1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim CveOrigen As String = TryCast(e.Row.FindControl("CveOrigen"), Label).Text
            ...
            'pnl_action.Visible = If(CveOrigen = "1", False, True)
            IBEliminar.Visible = If(Dependencias = "N" And CveOrigen <> "1", True, False)
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New Clientes().FindAllClientesPlataforma(CInt(Plataforma), "")
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Sub Exportar()
        ...
        Try
            ...
            dtgv.AsEnumerable.ToList.ForEach(Sub(r)
                                                 ...
                                                 lst.Add(String.Format("&nbsp;{0}", r("CodCliente")))
                                                 lst.Add(r("NomCliente"))
                                                 lst.Add(r("NomClienteA"))
                                                 lst.Add(r("Pais"))
                                                 lst.Add(r("NomOrigen"))
    ...
    Sub Agregar()
        ...
        Try
            ...
            Dim Valores As String = DDLClienteE.SelectedValue + "|" + Plataforma
            Dim ObjM As New Clientes()
            IsResult = ObjM.SaveModel_AdminCliente(-1, Valores, ObjUser.userId)
            If IsResult Then
                ObtenDatos()
                Alertas("", "6", False, 2)
            Else
        ...
        Try
            ...
            lstFiltros.FindAll(Function(p) New ArrayList(excepciones.Split(",")).IndexOf(p.CveControl.ToString) < 0).ForEach(Sub(p)
                                                                                                                                 ...
                                                                                                                                 ElseIf p.Valor <> "" And New ArrayList({"31", "41"}).IndexOf(p.CveTipo.ToString) >= 0 Then
                                                                                                                                     filtro += " AND [" + p.Campo + "]='" + p.Valor + "'"
                                                                                                                                 ElseIf p.Valor <> "" And New ArrayList({"32"}).IndexOf(p.CveTipo.ToString) >= 0 Then
    ...
    Sub mpe_open()
        ...
        MPEbody_Seleccionar.Visible = False
        BTNP15.Visible = True
        ...
        Select Case mpe_op.Text
            ...
            Case "action_add"
                MPEbody_Seleccionar.Visible = True
                lblMPE_title.Text = "Clientes"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 140
                BTNP16.Visible = True
