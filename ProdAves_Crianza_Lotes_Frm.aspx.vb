...
Public Class ProdAves_Crianza_Lotes_Frm
    Inherits Page
    Private ObjUser As UsuarioModel
    Private Plataforma As String = "54"
    Private menu As String = "23"
    Private menuC As String = "231"
    'Variables Generales
    ...
    Sub Etiquetas()
        ...
        'Titulo
        PageTitulo.Text = lstEtiquetas.Find(Function(p) p.CvePlataforma = CInt(Plataforma) And p.CveMenu = CInt(menu) And p.CveEtiqueta = 1).NomEtiqueta 'lstMenu.NomMenu
        'New Menu().FindById(ObjUser.CveRol, CInt(Plataforma), -1, menu).NomMenu
    ...
    Sub Controles(op As Boolean, Optional controles As String = "")
        Try
            Dim excepciones As String = "54-231-1,54-231-20,54-231-21,54-231-22,54-231-23,54-231-24"
    ...
    Sub ControlesAcciones(op As Integer, Optional excepciones As String = "")
        Try
            lstControles.FindAll(Function(p) p.CveEtapa > 0 And New ArrayList(excepciones.Split(",")).IndexOf(p.CveControl.ToString) < 0) _
                .ForEach(Sub(p)
    ...
    End Sub
    Sub LlenaDDL()
        Call New Acceso().LlenaClientes(DDLCliente, Plataforma, ObjUser.CodUsuario, 1)
        Call New Catalogos().Llena_ProdAvesEtapas(DDLEtapa)
        Call New Catalogos().Llena_ProdAvesLoteOrigen(DDLCveLoteO, DDLCliente.SelectedValue, CInt(regPId.Text))
        Call New Catalogos().LlenaProdAves_Granjas(DDLGranja, DDLCliente.SelectedValue)
        Call New Catalogos().Llena_ProdAvesLoteLineaG(DDLLineaG, DDLCliente.SelectedValue)
        Call New Catalogos().Llena_ProdAvesLoteGuia(DDLGuia, If(DDLLineaG.SelectedValue.ToString = "", 0, DDLLineaG.SelectedValue.ToString))
        Call New Catalogos().Llena_ProdAvesEstatus(DDLEstatus)
    End Sub
    Protected Sub DDLCliente_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDLCliente.SelectedIndexChanged
        CodCliente.Text = DDLCliente.SelectedValue.ToString
        Call New Catalogos().Llena_ProdAvesLoteOrigen(DDLCveLoteO, DDLCliente.SelectedValue, CInt(regPId.Text))
        Call New Catalogos().LlenaProdAves_Granjas(DDLGranja, DDLCliente.SelectedValue)
        Call New Catalogos().Llena_ProdAvesLoteLineaG(DDLLineaG, DDLCliente.SelectedValue)
        Call New Catalogos().Llena_ProdAvesLoteGuia(DDLGuia, If(DDLLineaG.SelectedValue.ToString = "", 0, DDLLineaG.SelectedValue.ToString))
    End Sub
    Protected Sub DDLLineaG_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDLLineaG.SelectedIndexChanged
        Call New Catalogos().Llena_ProdAvesLoteGuia(DDLGuia, If(DDLLineaG.SelectedValue.ToString = "", 0, DDLLineaG.SelectedValue.ToString))
    End Sub
    ...
    Sub SeguridadLoad()
        ...
        Dim IsEditable As Boolean = If(New ArrayList({"1"}).IndexOf(DDLEstatus.SelectedValue.ToString) >= 0, True, False)
        ...
        If regPId.Text = "0" Then 'Blanco
            ...
            DDLCliente.Visible = ClienteEditable
            TBNomClienteD.Visible = Not ClienteEditable
        ElseIf regPId.Text <> "0" And IsEditable Then
            Acciones(True, True, "0,1")
            Controles(True)
        ...
        Else
            ...
            Controles(False)
        End If
        DDLEtapa.Visible = False
        TBNomEtapaD.Visible = True
        DDLCveLoteO.Enabled = False
    ...
    Sub LlenaRegistro()
        Try
            ...
            If regPId.Text = "0" Then
                ...
                TBActualizaD.Text = ObjUser.NomUsuario + " | " + Now.ToString("dd/MM/yyyy")
                DDLEstatus.SelectedValue = 1
                DDLEtapa.SelectedValue = 1
                TBNomEtapaD.Text = DDLEtapa.SelectedItem.Text ' New Catalogos().Obten_ProdAvesEtapas.Find(Function(p) p.Id = 1).Nombre
            Else
                Dim ObjM As ProdAves_LotesModel = New ProdAves_Lotes().FindById(CodCliente.Text, CInt(regPId.Text), 0, 0)
                ...
                lstControles.FindAll(Function(p) p.CveEtapa > 0 And New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                    .ForEach(Sub(c)
                                 ...
                                 If c.CveMSC = "54-231-4" Then
                                     Call New Catalogos().Llena_ProdAvesLoteOrigen(DDLCveLoteO, DDLCliente.SelectedValue, CInt(regPId.Text))
                                     Call New Catalogos().LlenaProdAves_Granjas(DDLGranja, DDLCliente.SelectedValue)
                                     Call New Catalogos().Llena_ProdAvesLoteLineaG(DDLLineaG, DDLCliente.SelectedValue)
                                 ElseIf c.CveMSC = "54-231-12" Then
                                     Call New Catalogos().Llena_ProdAvesLoteGuia(DDLGuia, If(DDLLineaG.SelectedValue.ToString = "", 0, DDLLineaG.SelectedValue.ToString))
                                 End If
                             End Sub)
                CodCliente.Text = DDLCliente.SelectedValue.ToString
                TBActualizaD.Text = ObjM.NomUsuAct + " | " + ObjM.FecAct
                TBAvesInicialesD.Text = CInt(TBAvesInicialesD.Text).ToString("N0")
                TBAvesFinalesD.Text = CInt(TBAvesFinalesD.Text).ToString("N0")
            End If
            SeguridadLoad()
        Catch ex As Exception
            Alertas("", CleanSpecialCharacter(ex.Message), False, 4)
    ...
    '--Acciones---
    Sub Regresar()
        menu = If(DDLEstatus.SelectedValue.ToString = "2", "22", "21")
    ...
    End Sub
    Sub Refrescar()
        Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-" + menu + "-1").PaginaURL.Replace("@Id", Codif(CodCliente.Text + "|" + regPId.Text)).Replace("@filtro", Codif(filtroview.Text)).Replace("@pageIndex", gvindexpage.Text), True)
    End Sub
    ...
    Function GuardaDatos() As Boolean
        ...
        Try
            ...
            Dim lst = lstControles.FindAll(Function(p) p.Editable = "S" Or p.Obligatorio = "S").Select(Function(a) a.Valor).ToList()
            lst(3) = If(lst(3) = "", "0", lst(3))
            lst(7) = CDate(lst(7)).ToString("yyyy-MM-dd")
            Dim Valores As String = regPId.Text + "|" + String.Join("|", lst)
            'Dim Valores As String = String.Join("|", (lstControles.FindAll(Function(p) p.Editable = "S").Select(Function(a) a.Valor).ToList()))
            Dim ObjM As New ProdAves_Lotes()
            IsResult = ObjM.SaveModel(Valores, ObjUser.CodUsuario)
            mpe_op.Text = ""
            If IsResult Then
                If regPId.Text = "0" Then
