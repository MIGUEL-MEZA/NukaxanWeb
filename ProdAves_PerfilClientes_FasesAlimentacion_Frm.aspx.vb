...
Imports AjaxControlToolkit.Bundling
Imports Newtonsoft.Json
Imports NukaxanWEB.Libreria
Imports Saplin

Public Class ProdAves_PerfilClientes_FasesAlimentacion_Frm
    Inherits Page
    Private ObjUser As UsuarioModel
    Private Plataforma As String = "54"
    Private menu As String = "45"
    Private menuP As String = "4"
    'Variables Generales
    ...
    Public Sub LlenaMenu()
        ...
        Call New Menu_Formulario().LlenaMenu(MenuF, Plataforma, menuP, "", filename, pathURL)
    ...
    Sub Etiquetas()
        ...
        'Titulo
        PageTitulo.Text = New Menu().FindById(ObjUser.CveRol, CInt(Plataforma), -1, menuP).NomMenu
        'Menu_Titulo.Text = New Menu_Formulario().FindById(Plataforma, CInt(menu), "").NomMenu
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().Llena_ProdAvesEtapas(DDLEtapaE)
    ...
    Sub ControlesAcciones(op As Integer)
        Try
            Dim controles_cero As String = "1"
            Dim controles_mayuscula As String = "2"
            lstControles.FindAll(Function(p) p.CveEtapa = 1) _
                .ForEach(Sub(p)
                             Select Case p.CveTipo
                                 Case 1
                                     If op = 0 Then TryCast(UPContenido.FindControl(p.Control), Label).Text = ""
                                     If op = 0 And New ArrayList(controles_cero.Split(",")).IndexOf(p.CveControl.ToString) >= 0 Then TryCast(UPContenido.FindControl(p.Control), Label).Text = "0"
                                 ...
                                 Case 2
                                     ...
                                     If op = 1 And New ArrayList(controles_mayuscula.Split(",")).IndexOf(p.CveControl.ToString) >= 0 Then p.Valor = TryCast(UPContenido.FindControl(p.Control), TextBox).Text.ToUpper
                                 Case 3, 31, 32
    ...
    Sub LlenaRPT()
        Try
            rptRegistros.DataSource = New ProdAves_PerfilCliente_FasesAlimentacion().FindAll(regPId.Text, 0, 0)
    ...
    '--Acciones---
    Sub Regresar()
        Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-3-0").PaginaURL.Replace("@filtro", Codif(filtroview.Text)).Replace("@pageIndex", gvindexpage.Text), True)
    End Sub
    Sub Refrescar()
        Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-" + menu + "-1-6").PaginaURL.Replace("@Id", Codif(regPId.Text)).Replace("@filtro", Codif(filtroview.Text)).Replace("@pageIndex", gvindexpage.Text), True)
    End Sub
    Sub AbrirAgregar()
    ...
    Sub Guardar()
        ...
        Try
            ...
            Dim Valores As String = regPId.Text + "|" + String.Join("|", (lstControles.Select(Function(a) a.Valor).ToList()))
            Dim ObjM As New ProdAves_PerfilCliente_FasesAlimentacion()
            IsResult = ObjM.SaveModel(If(CInt(mpe_regId2.Text) = 0, 0, 1), Valores, ObjUser.CodUsuario)
            If IsResult Then
                LlenaRPT()
                Alertas("", New Mensajes().FindById("0", 2, 6).NomMensaje, 1, 2)
            Else
    ...
    Sub Editar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        mpe_clean()
        mpe_regId.Text = key.Split("-")(0)
        mpe_regId2.Text = key.Split("-")(1)
        mpe_open()
        mpe_fill()
    End Sub
    Sub Eliminar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            Dim ObjM As New ProdAves_PerfilCliente_FasesAlimentacion()
            Dim Valores As String = regPId.Text + "|" + key.Split("-")(1) + "|" + key.Split("-")(0)
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = "Nueva Granja"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 400
                BTNP16.Visible = True
            ...
            Case "action_edit"
                MPEBody_Captura.Visible = True
                lblMPE_title.Text = "Editar Granja"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 400
                BTNP16.Visible = True
                BTNP16.Text = lstAcciones.Find(Function(p) p.CveTipo = 1 And p.CveAccion = 6).NomAccion
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As ProdAves_PerfilCliente_FasesAlimentacionModel = New ProdAves_PerfilCliente_FasesAlimentacion().FindById(regPId.Text, CInt(mpe_regId.Text), CInt(mpe_regId2.Text))
