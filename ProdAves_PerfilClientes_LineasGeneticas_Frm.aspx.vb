...
Public Class ProdAves_PerfilClientes_LineasGeneticas_Frm
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "CveLineaG", .Valida = "N", .ValidaCeros = "N"})
    ...
    Sub LlenaRPT()
        Try
            rptRegistros.DataSource = New ProdAves_PerfilCliente_LineasGeneticas().FindAll(regPId.Text)
            rptRegistros.DataBind()
            rptRegistros.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
                                                                          Dim IsAdm As Boolean = If(New ArrayList({"1", "2"}).IndexOf(ObjUser.CveRol.ToString) >= 0, True, False)
                                                                          Dim CveLineaG As String = (TryCast(r.FindControl("CveLineaG"), Label)).Text
    ...
    Function Valida() As Boolean
        ...
        Try
            If msg = "" Then
                If ValidRPTChkCaptura(rptRegistros, "chk", IList) Then msg = New Mensajes().FindById("0", 0, 47).NomMensaje
            End If
    ...
    Function GuardaDatos() As Boolean
        ...
        Try
            ...
            Dim valores As String = GetRPTChkCaptura(rptRegistros, "chk", iList)

            Dim ObjM As New ProdAves_PerfilCliente_LineasGeneticas()
            IsResult = ObjM.SaveModel(regPId.Text, valores, ObjUser.CodUsuario)
            mpe_op.Text = ""
            If IsResult Then
                Alertas("", "6", True, 2)
            Else
    ...
    Protected Sub OnCheckedAllChanged(sender As Object, e As EventArgs)
        ...
        rptRegistros.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(p)
