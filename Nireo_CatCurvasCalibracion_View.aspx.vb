...
Public Class Nireo_CatCurvasCalibracion_View
    Inherits Page
    Private ObjUser As UsuarioModel
    Private Plataforma As String = "2"
    Private menu As String = "44"
    ...
    Sub Etiquetas()
        ...
        For Each a As Controles_AccionesModel In lstAcciones
            Select Case a.CveTipo
                ...
                Case 3  'ImageButton
                    Dim IB As ImageButton = TryCast(UPContenido.FindControl("IB" + a.CveTipo.ToString + a.CveAccion.ToString), ImageButton)
    ...
    Sub ControlesAcciones(op As Integer)
        Try
            lstControles.FindAll(Function(p) p.CveEtapa = 1 And p.Editable = "S") _
                .ForEach(Sub(p)
    ...
    Sub LlenaDDL()
        ...
        Call New Catalogos().LlenaNireo_CategoriaP(DDLFiltroCategoriaP)
        Call New Catalogos().LlenaNireo_Parametros(DDLFiltroParametro)
        Call New Catalogos().LlenaOpcionGeneral(DDLFiltroCritico, "1")

        Call New CatCategoriaProductos().LlenaCategoriaP(DDLCategoriaPE)
        Call New Catalogos().LlenaNireo_ParametrosxCategoria(DDLParametroE, DDLCategoriaPE.SelectedValue.ToString)
        Call New Catalogos().LlenaOpcionGeneral(DDLCriticoE, "1")
        Call New Catalogos().LlenaEstatusGeneral(DDLEstatusE)
    End Sub
    Protected Sub DDLCategoriaPE_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDLCategoriaPE.SelectedIndexChanged
        Try
            Call New Catalogos().LlenaNireo_ParametrosxCategoria(DDLParametroE, DDLCategoriaPE.SelectedValue.ToString)
            If DDLCategoriaPE.SelectedValue.ToString <> "" Then
                Dim ObjMC As CatCategoriaProductosModel = New CatCategoriaProductos().FindById(CInt(DDLCategoriaPE.SelectedValue.ToString))
                DDLCriticoE.Enabled = If(ObjMC.CveTipoP = 2, False, True)
                TBValorMinE.Enabled = If(ObjMC.CveTipoP = 2, False, True)
                TBValorMaxE.Enabled = If(ObjMC.CveTipoP = 2, False, True)
            End If
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            For Each a As Controles_AccionesModel In lstAcciones.Where(Function(p) p.CveTipo = 3)
                ...
                If Not IB Is Nothing Then
                    ...
                    IB.ToolTip = a.ToolTip
                    If a.ValidaClick = "S" Then IB.OnClientClick = "return confirm('" + a.ValidaMensaje + "');"
                    If IB.ID = "IB2" Then IB.Visible = If(Dependencias = "S", False, True)
                End If
            Next
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New Nireo_CatCurvasCalibracion().FindAll(0, 0)
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Sub Exportar()
        ...
        Try
            ...
            dtgv.AsEnumerable.ToList.ForEach(Sub(r)
                                                 ...
                                                 lst.Add(r("NomCategoriaP"))
                                                 lst.Add(r("NomParametro"))
                                                 lst.Add(r("Archivo"))
                                                 lst.Add(r("NomCritico"))
                                                 lst.Add(r("MinAceptado"))
                                                 lst.Add(r("MaxAceptado"))
    ...
    Function Valida() As Boolean
        ...
        Try
            ...
            lst = lstControles.FindAll(Function(c) c.CveEtapa = 1 And c.Editable = "S" And c.Obligatorio = "S" And New ArrayList(excepciones.Split(",")).IndexOf(c.CveMSC.ToString) < 0 And c.Valor = "").FirstOrDefault
            If Not lst Is Nothing Then msg = lst.Mensaje

            If msg = "" And FileUpload1.HasFile = False And TBArchivoE.Text = "" Then
                msg = "Debes seleccionar un modelo de curva."
            ElseIf msg = "" And FileUpload1.HasFile And TBArchivoE.Text = "" Then
                Dim postedFile As HttpPostedFile = FileUpload1.PostedFile
                If postedFile.ContentLength > 0 Then
                    Dim tmpExt As String = Path.GetExtension(postedFile.FileName).ToLower
                    If tmpExt <> ".unsb" Then
            ...
            If msg <> "" Then
                'Alertas("", msg, False, 4)
                ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Validationes", "alert('" + msg + "');", True)
            Else
                IsResult = True
            End If

        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Validationes", "alert('" + ex.Message.Replace("'", "") + "');", True)
            'Alertas("", CleanSpecialCharacter(ex.Message), False, 4)
        End Try
    ...
    Sub Guardar()
        ...
        Try
            ...
            If TBArchivoE.Text = "" And FileUpload1.HasFile Then
                Dim postedFile As HttpPostedFile = FileUpload1.PostedFile
                Dim FileExt As String = Path.GetExtension(postedFile.FileName).ToLower
                Dim FileSize As String = Math.Round(postedFile.ContentLength / CDec(1024), 2).ToString
                Dim FileName As String = postedFile.FileName

                Dim ObjM As New Nireo_CatCurvasCalibracion()
                opcion = If(mpe_op.Text = "action_add", 0, 1)
                Dim Valores As String = FileName + "|" + DDLCriticoE.SelectedValue + "|" + TBValorMinE.Text + "|" + TBValorMaxE.Text + "|" + DDLEstatusE.SelectedValue.ToString
                IsResult = ObjM.SaveModel(opcion, DDLCategoriaPE.SelectedValue, DDLParametroE.SelectedValue, Valores, ObjUser.userId)
                If IsResult Then
                    Dim FilePath = HttpContext.Current.Server.MapPath("~/CurvasModelo/") + FileName
                    Dim FileExist As Boolean = File.Exists(FilePath)
                    If FileExist Then File.Delete(FilePath)
            ...
            Else
                Dim ObjM As New Nireo_CatCurvasCalibracion()
                opcion = If(mpe_op.Text = "action_add", 0, 1)
                Dim Valores As String = TBArchivoE.Text + "|" + DDLCriticoE.SelectedValue + "|" + TBValorMinE.Text + "|" + TBValorMaxE.Text + "|" + DDLEstatusE.SelectedValue.ToString
                IsResult = ObjM.SaveModel(opcion, DDLCategoriaPE.SelectedValue, DDLParametroE.SelectedValue, Valores, ObjUser.userId)
                If IsResult Then
                    ObtenDatos()
                    mpe_close()
    ...
    Sub Eliminar(ByVal sender As Object, ByVal e As EventArgs)
        Dim key() As String = sender.CommandArgument.split("-")
        Dim IsResult As Boolean = False
        Try
            Dim FilePath = HttpContext.Current.Server.MapPath("~/CurvasModelo/") + key(2)
            Dim FileExist As Boolean = File.Exists(FilePath)
            Dim ObjM As New Nireo_CatCurvasCalibracion()
            IsResult = ObjM.DeleteModel(key(0), key(1), ObjUser.userId)
            If IsResult Then
                If FileExist Then File.Delete(FilePath)
                ObtenDatos()
                mpe_close()
    ...
    Sub mpe_clean()
        ...
        Select Case mpe_op.Text
            ...
            Case "action_add", "action_edit"
                ControlesAcciones(0)
                DDLCategoriaPE.SelectedValue = ""
                Call New Catalogos().LlenaNireo_ParametrosxCategoria(DDLParametroE, DDLCategoriaPE.SelectedValue.ToString)
                'DDLParametroE.SelectedValue = ""
                DDLCriticoE.SelectedValue = "N"
                TBValorMinE.Text = 0
                TBValorMaxE.Text = 0
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            ...
            Case "action_add", "action_edit"
                ...
                lblMPE_title.Text = "CURVAS DE CALIBRACIÓN"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 500
                BTNP16.Visible = True
                ...
                DDLCategoriaPE.Enabled = True
                DDLParametroE.Enabled = True
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Call New Catalogos().LlenaNireo_Parametros(DDLParametroE)
                    Dim tmp() As String = mpe_regId.Text.Split("-")
                    Dim ObjM As Nireo_CatCurvasCalibracionModel = New Nireo_CatCurvasCalibracion().FindById(CInt(tmp(0)), CInt(tmp(1)))
                    Dim excepciones As String = ""
                    lstControles.FindAll(Function(p) New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                        .ForEach(Sub(c)
                                 ...
                                 End Sub)
                    sec_archivo.Visible = True
                    FileUpload1.Visible = False

                    DDLCategoriaPE.Enabled = False
                    DDLParametroE.Enabled = False
                    DDLEstatusE.Enabled = False

                    'DDLCriticoE.Enabled = If(ObjM.CveTipoP = 2, False, True)
                    'TBValorMinE.Enabled = If(ObjM.CveTipoP = 2, False, True)
                    'TBValorMaxE.Enabled = If(ObjM.CveTipoP = 2, False, True)
