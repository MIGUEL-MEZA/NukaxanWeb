...
Public Class Nireo_CatProductos_Parametros_View
    Inherits Page
    Private ObjUser As UsuarioModel
    Private Plataforma As String = "2"
    Private menu As String = "47"
    ...
    End Sub
    Sub LlenaDDL()
        Call New CatCategoriaProductos().LlenaCategoriaP(DDLFiltroCategoriaP)
        Call New CatParametros().LlenaParametros(DDLFiltroParametro)
    ...
    Sub LLenaChk()
        Dim dt As DataTable
        Try
            dt = New CatParametros().ObtieneParametros(If(DDLCategoriaPE.SelectedValue.ToString = "", 0, DDLCategoriaPE.SelectedValue))
            ChkParametrosE.DataSource = dt
            ChkParametrosE.DataTextField = "NomParametro"
            ChkParametrosE.DataValueField = "CveParametro"
            ChkParametrosE.DataBind()

            For Each chk In ChkParametrosE.Items
                Dim existe As String = dt.AsEnumerable().Where(Function(c) c("CveParametro") = chk.value).CopyToDataTable().Rows(0)(“Existe”).ToString
                chk.Selected = If(existe = "1", True, False)
            Next

        Catch ex As Exception
            ChkParametrosE.DataSource = Nothing
            ChkParametrosE.DataBind()
        Finally
            dt = Nothing
        End Try
    ...
    Protected Sub GVRowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gv1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim Critico As String = TryCast(e.Row.FindControl("Critico"), Label).Text
            Dim DDLCritico As DropDownList = TryCast(e.Row.FindControl("DDLCritico"), DropDownList)
            Call New Catalogos().LlenaOpcionGeneral(DDLCritico, "1")
            DDLCritico.SelectedValue = Critico
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New Nireo_CatProducto_Parametros().FindAll(0, 0)
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Function Valida() As Boolean
        ...
        Try
            If gv1.Rows.Cast(Of GridViewRow)().Where(Function(c) _
                                                         TryCast(c.FindControl("DDLCritico"), DropDownList).SelectedValue.ToString = "" _
                                                         Or TryCast(c.FindControl("TBMinValor"), TextBox).Text = "" _
                                                         Or TryCast(c.FindControl("TBMaxValor"), TextBox).Text = ""
                                                         ).Count > 0 Then
                mpe_notifications(lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 30).NomMensaje, 3)
            Else
                IsResult = True
            End If
    ...
    Sub Guardar()
        Dim IsResult As Boolean = False
        Try
            If DDLFiltroCategoriaP.SelectedValue.ToString = "" Then
                mpe_notifications(lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 29).NomMensaje, 3)
                Exit Sub
            ElseIf Valida = False Then
                Exit Sub
            End If

            Dim lstValores As New List(Of String)
            gv1.Rows.Cast(Of GridViewRow)().ToList.ForEach(Sub(r)
                                                               Dim CveCategoriaP As String = TryCast(r.FindControl("CveCategoriaP"), Label).Text
                                                               Dim CveParametro As String = TryCast(r.FindControl("CveParametro"), Label).Text
                                                               Dim Critico As String = TryCast(r.FindControl("DDLCritico"), DropDownList).SelectedValue.ToString
                                                               Dim TBMinValor As String = TryCast(r.FindControl("TBMinValor"), TextBox).Text
                                                               Dim TBMaxValor As String = TryCast(r.FindControl("TBMaxValor"), TextBox).Text
                                                               lstValores.Add(CveCategoriaP + "#" + CveParametro + "#" + Critico + "#" + TBMinValor + "#" + TBMaxValor)
                                                           End Sub)

            Dim ObjM As New Nireo_CatProducto_Parametros()
            IsResult = ObjM.SaveModel(String.Join("|", lstValores.ToArray()), ObjUser.userId)
            If IsResult Then
                FiltrosCancelar()
                ObtenDatos()
                mpe_notifications(lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 27).NomMensaje, 1)
            Else
    ...
    Sub Agregar()
        ...
        Try
            Dim ObjM As New Nireo_CatProducto_Parametros()
            Dim valores As String = String.Join("|", (ChkParametrosE.Items.Cast(Of ListItem)().Where(Function(i) i.Selected).[Select](Function(i) i.Value)).ToList())
            IsResult = ObjM.EditModel(DDLCategoriaPE.SelectedValue, valores, ObjUser.userId)
            If IsResult Then
                FiltrosCancelar()
                ObtenDatos()
                mpe_notifications(lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 27).NomMensaje, 1)
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            ...
            Case "action_edit"
                ...
                lblMPE_title.Text = "Editar Relación"
                pnlPopup.Width = 550
                pnlPopup.Height = 550
                BTNP16.Visible = True
            ...
            Select Case mpe_op.Text
                ...
                Case "action_edit"
                    If DDLCategoriaPE.SelectedValue.ToString = "" Then
                        msg = lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 29).NomMensaje
                    ElseIf ChkParametrosE.Items.Cast(Of ListItem)().Where(Function(c) c.Selected).Count = 0 Then
                        msg = lstMensajes.Find(Function(p) p.CveMenu = 0 And p.CveMensaje = 31).NomMensaje
                    End If
            End Select
