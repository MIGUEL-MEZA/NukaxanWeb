...
Public Class ProdAves_Crianza_Lotes_Recepcion_Frm
    ...
    Sub Controles(op As Boolean, Optional controles As String = "")
        Try
            Dim excepciones As String = "54-212-5"
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().LlenaProdAves_Casetas(DDLCasetaE, CodCliente.Text, CInt(CveGranja.Text))
        Call New Catalogos().Llena_ProdAvesEstatus(DDLEstatusE)
    End Sub
    ...
    Sub LlenaRPT()
        Try
            rptRegistros.DataSource = New ProdAves_Lotes_Recepcion().FindAll(CodCliente.Text, CInt(regPId.Text), 0)
    ...
    '--Acciones---
    Sub Regresar()
        menu = If(CveEstatus.Text = "2", "22", "21")
    ...
    Sub Guardar()
        ...
        Try
            ...
            Dim lst = lstControles.FindAll(Function(p) p.Editable = "S").Select(Function(a) a.Valor).ToList()
            lst(1) = CDate(lst(1)).ToString("yyyy-MM-dd")
            Dim Valores As String = CodCliente.Text + "|" + regPId.Text + "|" + mpe_regId.Text + "|" + String.Join("|", lst)
    ...
    Sub mpe_clean()
        ...
        TBEdadE.Text = "1"
        TBDiaE.Text = "1"
        TBEdadE.Enabled = False
        TBDiaE.Enabled = False
    End Sub
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = "Nueva Recepción"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 450
                BTNP16.Visible = True
            ...
            Case "action_edit"
                MPEBody_Captura.Visible = True
                lblMPE_title.Text = "Editar Recepción"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 450
                BTNP16.Visible = True
                BTNP16.Text = lstAcciones.Find(Function(p) p.CveTipo = 1 And p.CveAccion = 6).NomAccion
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As ProdAves_Lotes_RecepcionModel = New ProdAves_Lotes_Recepcion().FindById(CodCliente.Text, CInt(regPId.Text), CInt(mpe_regId.Text))
