...
Public Class ProdAves_Postura_Lotes_Registro_Frm
        ...
        Try
            ...
            .ForEach(Sub(p)
                         Select Case p.CveTipo
                             ...
                             Case 2
                                 If op = 0 Then TryCast(UPContenido.FindControl(p.Control), TextBox).Text = "0"
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().Llena_ProdAvesCiclo(DDLCicloE, CodCliente.Text)
        Call New Catalogos().Llena_ProdAvesLoteRecepcion(DDLLoteRecepcion, CodCliente.Text, CInt(regPId.Text))
        DDLLoteRecepcion.SelectedIndex = 1
        LlenaRPT()
    End Sub
    Sub DDLLoteRecepcion_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDLLoteRecepcion.SelectedIndexChanged
        LlenaRPT()
    End Sub
    ...
    Sub LlenaRegistro()
        Try
            ...
            PageTitulo.Text = "LOTE: " + Obj.CodLote 'New Clientes().FindById(CodCliente.Text).NomClienteR
            SeguridadLoad()
        Catch ex As Exception
            Alertas("", CleanSpecialCharacter(ex.Message), False, 4)
        End Try
    ...
    Sub LlenaRPT()
        Try
            rptRegistros.DataSource = New ProdAves_Lotes_Registro_Captura().FindAll(CodCliente.Text, CInt(regPId.Text), If(DDLLoteRecepcion.SelectedValue.ToString = "", -1, DDLLoteRecepcion.SelectedValue), 0)
            ...
            rptRegistros.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
                                                                          ...
                                                                          Dim FolioR As Label = TryCast(r.FindControl("FolioR"), Label)
                                                                          FolioR.Style.Add("cursor", "pointer")
    ...
    Sub LlenaDT()
        Try
            DLProduccion.DataSource = New ProdAves_Lotes_Registro_ClasificacionH().FindAll(CodCliente.Text, CInt(regPId.Text), If(DDLLoteRecepcion.SelectedValue.ToString = "", -1, DDLLoteRecepcion.SelectedValue), CInt(mpe_regId.Text), 0)
            DLProduccion.DataBind()

        Catch ex As Exception
            DLProduccion.DataSource = Nothing
            DLProduccion.DataBind()
            Alertas("", CleanSpecialCharacter(ex.Message), False, 4)
        End Try
    End Sub
    ...
    Sub AbrirAgregar()
        ...
        mpe_clean()
        mpe_open()
        Dim Obj As ProdAves_Lotes_Registro_CapturaModel = New ProdAves_Lotes_Registro_Captura().FindlstAll(CodCliente.Text, CInt(regPId.Text), If(DDLLoteRecepcion.SelectedValue.ToString = "", -1, DDLLoteRecepcion.SelectedValue), 0).LastOrDefault
        If IsNothing(Obj) Then
            Dim ObjR As ProdAves_Lotes_RecepcionModel = New ProdAves_Lotes_Recepcion().FindById(CodCliente.Text, CInt(regPId.Text), If(DDLLoteRecepcion.SelectedValue.ToString = "", -1, DDLLoteRecepcion.SelectedValue))
            TBFechaE.Text = ObjR.FecRecepcion
            TBEdadE.Text = ObjR.Edad.ToString
            TBNumDiaE.Text = ObjR.NumDia.ToString
        Else
            TBFechaE.Text = CDate(Obj.FecCaptura).AddDays(1).ToString("dd/MM/yyyyy")
            TBEdadE.Text = If(Obj.NumDia = 7, Obj.Edad + 1, Obj.Edad).ToString
            TBNumDiaE.Text = If(Obj.NumDia = 7, 1, Obj.NumDia + 1).ToString
        End If
        LlenaDT()
    End Sub
    Sub Guardar()
        ...
        Try
            ...
            Dim lst = lstControles.FindAll(Function(p) p.Obligatorio = "S").Select(Function(a) a.Valor).ToList()
            lst(2) = CDate(lst(2)).ToString("yyyy-MM-dd")
            Dim Valores As String = CodCliente.Text + "|" + regPId.Text + "|" + DDLLoteRecepcion.SelectedValue.ToString + "|" + mpe_regId.Text + "|" + String.Join("|", lst)
            Dim ValoresH As String = String.Join("|", DTObtenCaptura().ToArray())


            Dim ObjM As New ProdAves_Lotes_Registro_Captura
            IsResult = ObjM.SaveModel(If(mpe_regId.Text = "0", "0", "1"), Valores, ValoresH, ObjUser.CodUsuario)
            If IsResult Then
                LlenaRPT()
                Alertas("", New Mensajes().FindById("0", 0, 6).NomMensaje, False, 2)
            Else
    ...
    Sub Editar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        mpe_fill()
        LlenaDT()
        'DDLEstatusE.Enabled = True
    End Sub
    Sub Eliminar(ByVal sender As Object, ByVal e As EventArgs)
        Dim key As String = sender.CommandArgument
        Dim IsResult As Boolean = False
        Try
            Dim ObjM As New ProdAves_Lotes_Recepcion()
            Dim Valores As String = CodCliente.Text + "|" + regPId.Text + "|" + key
            IsResult = ObjM.DeleteModel(Valores)
            If IsResult Then
                LlenaRPT()
                mpe_op.Text = ""
                Alertas("", "17", False, 2)
    ...
    Function DTObtenCaptura() As List(Of String)
        Dim lstEtapas As New List(Of String)
        Try
            DLProduccion.Items.Cast(Of DataListItem)().ToList.ForEach(Sub(p)
                                                                          Dim CveClasificacionH As String = TryCast(p.FindControl("CveClasificacionH"), Label).Text
                                                                          Dim Total As Double = CDbl(TryCast(p.FindControl("TBTotalE"), TextBox).Text)
                                                                          Dim PiezasCaja As Double = CDbl(TryCast(p.FindControl("PiezasCaja"), Label).Text)
                                                                          Dim ConosCaja As Double = CDbl(TryCast(p.FindControl("ConosCaja"), Label).Text)
                                                                          Dim PiezasCono As Double = CDbl(TryCast(p.FindControl("PiezasCono"), Label).Text)

                                                                          Dim Total_Cajas As Integer = 0
                                                                          Dim Total_Conos As Integer = 0
                                                                          Dim Total_Piezas As Integer = 0

                                                                          Total_Conos = If(Total >= PiezasCono, Fix(Total / PiezasCono), 0)
                                                                          Total_Piezas = Total - (Total_Conos * PiezasCono)
                                                                          Total_Cajas = If(Total_Conos = 0, 0, Fix(Total_Conos / ConosCaja))
                                                                          Total_Conos = If(Total_Conos = 0, 0, Total_Conos - Fix(Total_Cajas * ConosCaja))

                                                                          lstEtapas.Add(CveClasificacionH + "#" + Total.ToString + "#" + Total_Cajas.ToString + "#" + Total_Conos.ToString + "#" + Total_Piezas.ToString)
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = PageTitulo.Text
                pnlCaptura.Width = 750
                pnlCaptura.Height = 550
                BTNP16.Visible = True
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As ProdAves_Lotes_Registro_CapturaModel = New ProdAves_Lotes_Registro_Captura().FindById(CodCliente.Text, CInt(regPId.Text), DDLLoteRecepcion.SelectedValue, CInt(mpe_regId.Text))
                    Dim excepciones As String = "54-334-14"
                    lstControles.FindAll(Function(p) New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                        .ForEach(Sub(c)
