...
Public Class Nireo_Clientes_Equipos_Frm
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "CveEquipo", .Valida = "N", .ValidaCeros = "N"})
        iList.Add(New DatosGrid() With {.Tipo = 1, .Campo = "TBFecAsignacion", .Valida = "S", .ValidaCeros = "N", .IsDate = "S"})
    ...
    Sub Etiquetas()
        ...
        Menu_Titulo.Text = New Menu_Formulario().FindById(Plataforma, menu, 5).NomMenu
    ...
    Sub LlenaRPT()
        Try
            rptEquipos.DataSource = New Clientes_Equipos().FindAll(regPId.Text)
            rptEquipos.DataBind()
            rptEquipos.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
    ...
    End Sub
    Sub Refrescar()
        Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-" + menu + "-1-5").PaginaURL.Replace("@Id", Codif(regPId.Text)).Replace("@filtro", Codif(filtroview.Text)).Replace("@pageIndex", gvindexpage.Text), True)
    End Sub
    Function Valida() As Boolean
        Dim IsResult As Boolean = False
        Try
            If rptEquipos.Items.Count = 0 Then msg = New Mensajes().FindById("0", 0, 53).NomMensaje
            If msg = "" Then
                If ValidRPTCaptura(rptEquipos, iList) Then msg = New Mensajes().FindById("0", 0, 47).NomMensaje
            End If
    ...
    Function GuardaDatos() As Boolean
        ...
        Try
            ...
            Dim valores As String = GetRPTCaptura(rptEquipos, iList)

            'Alertas("", valores, False, 4)
            'Return False
            Dim ObjM As New Clientes_Equipos()
            IsResult = ObjM.UpdateModel(regPId.Text, valores, ObjUser.userId)
            mpe_op.Text = ""
            If IsResult Then
                'mpe_op.Text = "action_refresca"
                Alertas("", New Mensajes().FindById("0", 0, 6).NomMensaje, 1, 2)
            Else
    ...
    Sub Agregar()
        ...
        Try
            Dim ObjM As New Clientes_Equipos()
            IsResult = ObjM.SaveModel(regPId.Text, DDLEquipoPE.SelectedValue + "#" + Now.ToString("yyyy-MM-dd"), ObjUser.userId)
            If IsResult Then
                LlenaRPT()
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = "Selección Equipos"
