...
Public Class ProdAves_Postura_Lotes_ProgramaAlim_Frm
    Inherits Page
    Private ObjUser As UsuarioModel
    Private Plataforma As String = "54"
    Private menu As String = "33"
    Private menuC As String = "333"
    Private CveEtapa As Integer = 2
    'Variables Generales
    ...
    Sub Controles(op As Boolean, Optional controles As String = "")
        Try
            Dim excepciones As String = "54-333-1"
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().Llena_ProdAvesFases(DDLFaseE, CodCliente.Text, CveEtapa)
    ...
    Sub LlenaRPT()
        Try
            rptRegistros.DataSource = New ProdAves_Lotes_ProgramaA().FindAll(CodCliente.Text, CInt(regPId.Text), 0)
    ...
    Sub Guardar()
        ...
        Try
            ...
            Dim Valores As String = CodCliente.Text + "|" + regPId.Text + "|" + mpe_regId.Text + "|" + CveEtapa.ToString + "|" + String.Join("|", lst)
            Dim ObjM As New ProdAves_Lotes_ProgramaA()
            IsResult = ObjM.SaveModel(If(TBIDE.Text = "", 0, 1), Valores, ObjUser.CodUsuario)
            If IsResult Then
                LlenaRPT()
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As ProdAves_Lotes_ProgramaAModel = New ProdAves_Lotes_ProgramaA().FindById(CodCliente.Text, CInt(regPId.Text), CInt(mpe_regId.Text))
