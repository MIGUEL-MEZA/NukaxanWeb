...
Public Class Nireo_CatCategoriaProductos_View
    Inherits Page
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().LlenaProductos_Tipos(DDLTipoE)
        Call New Catalogos().LlenaEstatusGeneral(DDLEstatusE)
        Call New Catalogos().LlenaProductos_Tipos(DDLFiltroTipo)
    ...
    Sub ObtenDatos()
        ...
        Try
            gv1.PageIndex = 0
            dt = New CatCategoriaProductos().FindAll(0)
            Session("DatosGV") = dt
            LlenaGV(dt)
        Catch ex As Exception
    ...
    Sub LlenaFiltros()
        Try
            lstFiltros.ForEach(Sub(c)
                                   ...
                                   If c.CveTipo = 5 Then c.Valor = CType(UPContenido.FindControl(c.Control), Saplin.Controls.DropDownCheckBoxes).SelectedValue
    ...
    Sub Guardar()
        ...
        Try
            ...
            lstControles.FindAll(Function(p) p.Editable = "S").ForEach(Sub(c)
                                                                       ...
                                                                       End Sub)
            Dim ObjM As New CatCategoriaProductos()
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As CatCategoriaProductosModel = New CatCategoriaProductos().FindById(CInt(mpe_regId.Text))
                    Dim excepciones As String = ""
                    lstControles.FindAll(Function(p) New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                        .ForEach(Sub(c)
                                 ...
                                 End Sub)
                    'If ObjM.ImgProducto <> "" Then ImgEspecifica.ImageUrl = "./Content/Image/" + ObjM.ImgProducto
