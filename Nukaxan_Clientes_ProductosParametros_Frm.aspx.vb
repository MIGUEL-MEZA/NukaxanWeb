...
Public Class Nukaxan_Clientes_ProductosParametros_Frm
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "CveParametro", .Valida = "N", .ValidaCeros = "N"})
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "Aplica", .Valida = "N", .ValidaCeros = "N"})
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "Dependencias", .Valida = "N", .ValidaCeros = "N"})
        iList.Add(New DatosGrid() With {.Tipo = 2, .Campo = "DDLEstatusE", .Valida = "S", .ValidaCeros = "N"})
    ...
    Sub Etiquetas()
        ...
        Menu_Titulo.Text = New Menu_Formulario().FindById(Plataforma, menu, 7).NomMenu
    ...
    End Sub
    Sub LlenaDDL()
        'Call New Catalogos().LlenaCliente_Equipos(DDLEquipoPE, regPId.Text)
    ...
    Sub LlenaRPT()
        Try
            rptProductos.DataSource = New Clientes_Productos().FindAll(regPId.Text, 0)
            rptProductos.DataBind()
            rptProductos.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
                                                                          ...
                                                                          For Each a As Controles_AccionesModel In lstAcciones.Where(Function(p) p.CveTipo = 3)
                                                                              ...
                                                                              End If
                                                                          Next
                                                                          Dim CveProducto As String = TryCast(r.FindControl("CveProducto"), Label).Text
                                                                          Dim rpt As Repeater = TryCast(r.FindControl("rptParametros"), Repeater)
                                                                          Call LlenaRPT_Parametros(rpt, CveProducto)
                                                                      End Sub)
        Catch ex As Exception
        ...
        End Try
    End Sub
    Sub LlenaRPT_Parametros(rpt As Repeater, CveProducto As Integer)
        Try
            rpt.DataSource = New Clientes_ProductosParametros().FindAll(0, regPId.Text, CveProducto, 0)
            rpt.DataBind()
    ...
    Sub LlenaRPT_ParametrosE()
        Try
            rptParametrosE.DataSource = New Clientes_ProductosParametros().FindAll(1, regPId.Text, CInt(mpe_regId.Text), 0)
            rptParametrosE.DataBind()

            rptParametrosE.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
                                                                            Dim Aplica As String = TryCast(r.FindControl("Aplica"), Label).Text
                                                                            Dim Dependencias As String = TryCast(r.FindControl("Dependencias"), Label).Text
                                                                            ...
                                                                            Dim CveEstatus As String = (TryCast(r.FindControl("CveEstatus"), Label)).Text
                                                                            Dim DDLEstatus As DropDownList = (TryCast(r.FindControl("DDLEstatusE"), DropDownList))
                                                                            Call New Catalogos().LlenaEstatusGeneral(DDLEstatus)
                                                                            DDLEstatus.SelectedValue = If(CveEstatus = "0", "", CveEstatus)
                                                                            'DDLEstatus.Enabled = If(Dependencias = "S", False, True)
                                                                        End Sub)
        Catch ex As Exception
            rptParametrosE.DataSource = Nothing
            rptParametrosE.DataBind()
        End Try
    End Sub
    ...
    End Sub
    Sub Refrescar()
        Response.Redirect(New RedirectPaginas().FindById(Plataforma + "-" + menu + "-1-7").PaginaURL.Replace("@Id", Codif(regPId.Text)).Replace("@filtro", Codif(filtroview.Text)).Replace("@pageIndex", gvindexpage.Text), True)
    End Sub
    Function Valida() As Boolean
        Dim IsResult As Boolean = False
        Try
            If msg = "" Then
                If ValidRPTChkCaptura(rptParametrosE, "chk", iList) Then msg = New Mensajes().FindById("0", 0, 47).NomMensaje
            End If
    ...
    Function GuardaDatos() As Boolean
        ...
        Try
            ...
            Dim valores As String = GetRPTChkCaptura(rptParametrosE, "chk", iList)
            Dim ObjM As New Clientes_ProductosParametros()
            IsResult = ObjM.SaveModel(regPId.Text, CInt(mpe_regId.Text), valores, ObjUser.userId)
            If IsResult Then
                Alertas("", "52", False, 2)
                LlenaRPT()
            Else
    ...
    Sub Editar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        mpe_regId.Text = key
        mpe_open()
        LlenaRPT_ParametrosE()
    End Sub
    Sub ExpandirColapsar(ByVal sender As Object, ByVal e As EventArgs)
        Dim item As RepeaterItem = CType(sender.NamingContainer, RepeaterItem)
        Dim IBExpandir As ImageButton = TryCast(item.FindControl("IB3"), ImageButton)
        Dim IBColapsar As ImageButton = TryCast(item.FindControl("IB4"), ImageButton)
        Dim pnlParametros As Panel = TryCast(item.FindControl("pnlParametros"), Panel)
        Dim CveProducto As String = TryCast(item.FindControl("CveProducto"), Label).Text
        mpe_regId.Text = CveProducto
        Dim op As String = sender.CommandArgument

        IBExpandir.Visible = If(op = "E", False, True)
        IBColapsar.Visible = If(op = "E", True, False)
        pnlParametros.Visible = If(op = "E", True, False)
