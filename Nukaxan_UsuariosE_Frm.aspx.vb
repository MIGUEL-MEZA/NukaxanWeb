...
Public Class Nukaxan_UsuariosE_Frm
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "CodCliente", .Valida = "N", .ValidaCeros = "N"})
    ...
    Sub Controles(op As Boolean, Optional controles As String = "")
        Try
            Dim excepciones As String = "1-62-1,1-62-16"
    ...
    Sub LlenaDDL()
        ...
        Call New Catalogos().LlenaRoles(DDLRol, "E")
        Call New Catalogos().LlenaRoles(DDLRol, "E")
    End Sub
    ...
    Sub SeguridadLoad()
        ...
        Controles(False)
        bar_action.Visible = If(regPId.Text = "0", False, True)
    ...
    Sub LlenaRegistro()
        Try
            ...
            Else
                Dim ObjM As UsuarioModel = New Usuario().FindById(1, 0, regPId.Text)
                ...
                lstControles.FindAll(Function(p) p.CveEtapa > 0 And New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                    .ForEach(Sub(c)
                             ...
                             End Sub)
                'CveOrigen.Text = ObjM.CveOrigen.ToString
    ...
    Sub LlenaRPT()
        Try
            rptClientes.DataSource = New Clientes_UsuariosE().FindAll("", regPId.Text)
            rptClientes.DataBind()
            rptClientes.Items.Cast(Of RepeaterItem)().ToList.ForEach(Sub(r)
                                                                         Dim CodCliente As String = TryCast(r.FindControl("CodCliente"), Label).Text
                                                                         Dim Dependencias As String = TryCast(r.FindControl("Dependencias2"), Label).Text
                                                                         Dim IB2 As ImageButton = TryCast(r.FindControl("IB2"), ImageButton)
                                                                         IB2.Visible = If(Dependencias = "S" Or CodCliente = "", False, True)

                                                                         Dim CveEstatus As String = (TryCast(r.FindControl("CveEstatus"), Label)).Text
                                                                         Dim DDLEstatus As DropDownList = (TryCast(r.FindControl("DDLEstatus"), DropDownList))
                                                                         Call New Catalogos().LlenaEstatusGeneral(DDLEstatus)
                                                                         DDLEstatus.SelectedValue = If(CveEstatus = "0", "", CveEstatus)
                                                                         DDLEstatus.Visible = If(CodCliente = "", False, True)
                                                                     ...
                                                                     End Sub)
        Catch ex As Exception
            rptClientes.DataSource = Nothing
            rptClientes.DataBind()
            Alertas("", CleanSpecialCharacter(ex.Message), False, 4)
        End Try
    End Sub
    ...
    Function Valida() As Boolean
        ...
        Try
            ...
            If msg = "" And regPId.Text <> "0" And rptClientes.Items.Count = 0 Then msg = New Mensajes().FindById("0", 0, 49).NomMensaje

            If msg = "" And regPId.Text <> "0" And ValidRPTCaptura(rptClientes, iList) Then msg = New Mensajes().FindById("0", 0, 47).NomMensaje
    ...
    Function GuardaDatos() As Boolean
        Dim IsResult As Boolean = False
        Dim IsResult2 As Boolean = False
        Try
            ControlesAcciones(1)
            If Valida() = False Then Return False

            Dim Valores As String = TBCodigoD.Text + "|" + String.Join("|", (lstControles.FindAll(Function(p) p.CveEtapa > 0 And p.Editable = "S").Select(Function(a) a.Valor).ToList()))
            Dim ObjM As New Usuario()
            IsResult = ObjM.SaveModel(If(regPId.Text = "0", 0, 1), 0, "E", Valores, ObjUser.CodUsuario)

            If regPId.Text <> "0" Then
                Dim valores2 As String = GetRPTCaptura(rptClientes, iList)
                IsResult2 = New Clientes_UsuariosE().UpdateModel(regPId.Text, valores2, ObjUser.CodUsuario)
            End If
            If IsResult Then
                If regPId.Text = "0" Then
    ...
    Sub Agregar()
        Dim IsResult As Boolean = False
        Try
            Dim ObjM As New Clientes_UsuariosE()
            IsResult = ObjM.SaveModel(DDLClientePE.SelectedValue, regPId.Text, ObjUser.CodUsuario)
            If IsResult Then
                LlenaRPT()
                'Alertas("", New Mensajes().FindById("0", 0, 48).NomMensaje, 1, 2)
            Else
                'ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Validation", "alert('" + CleanSpecialCharacter(ObjM.strError) + "');", True)
                Alertas("", CleanSpecialCharacter(ObjM.strError), False, 4)
            End If
        Catch ex As Exception
    ...
    Sub Eliminar(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            Dim ObjM As New Clientes_UsuariosE()
            IsResult = ObjM.DeleteModel(key, regPId.Text)
            If IsResult Then
                LlenaRPT()
                mpe_op.Text = ""
                Alertas("", "17", False, 2)
    ...
    Sub mpe_clean()
        mpe_regId.Text = "0"
        Call New Catalogos().LlenaClientesAll(DDLClientePE, 1, TBCodigoD.Text, 1)
        DDLClientePE.SelectedValue = ""
    End Sub
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = "Selección Cliente"
