...
Public Class Master_ProdAves
    ...
    Sub Etiquetas()
        'Titulo
        'LBLEslogan.Text = New Plataforma().FindById(ObjUser.CveRol, 5, 0).Descripcion
        'LBLTitulo.Text = New Plataforma().FindById(ObjUser.CveRol, 54, 5).NomPlataforma

    End Sub
    Sub LlenaPlataformas()
        Try
            Dim lstP As List(Of PlataformaModel) = New Acceso().FindlstAllPlataformas(ObjUser.CveRol, 0, 0) _
            .Where(Function(p) p.CvePlataforma <> 5).ToList()
    ...
    Sub LlenaMenu(lst As List(Of MenuModel), parentMenuId As Integer, parentMenuItem As MenuItem)
        ...
        Try
            ...
            lst.ForEach(Sub(p)
                            ...
                            menuItem.Selected = If(p.Url.Contains("Nulink_ProdAves_PerfilClientes") And currentPage.Contains("Nulink_ProdAves_PerfilClientes"), True, False)
