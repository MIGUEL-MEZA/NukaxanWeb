...
Public Class Nukaxan_Clientes_Productos_Frm
    Inherits Page
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        lstControles = New Controles_Captura().FindlstAll(CInt(Plataforma), CInt(menu + "6"), 0)

        iList.Add(New DatosGrid() With {.Tipo = 0, .Campo = "CveEquipo", .Valida = "N", .ValidaCeros = "N"})
        iList.Add(New DatosGrid() With {.Tipo = 1, .Campo = "TBFecAsignacion", .Valida = "S", .ValidaCeros = "N"})
    ...
    Sub Etiquetas()
        ...
        Menu_Titulo.Text = New Menu_Formulario().FindById(Plataforma, menu, 6).NomMenu
    ...
    End Sub
    Sub LlenaDDL()
        Call New Catalogos().LlenaNireo_CategoriaP(DDLCategoriaE)
    ...
    Function Valida() As Boolean
        ...
        Try
            If msg = "" Then
                If ValidRPTCaptura(rptProductos, iList) Then msg = New Mensajes().FindById("0", 0, 47).NomMensaje
            End If
    ...
    Function GuardaDatos() As Boolean
        ...
        Try
            ...
            Dim valores As String = GetRPTCaptura(rptProductos, iList)
    ...
    Sub mpe_open()
        ...
        Select Case mpe_op.Text
            Case "action_add"
                ...
                lblMPE_title.Text = "Nuevo Producto"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 420
                BTNP16.Visible = True
            ...
            Case "action_edit"
                MPEBody_Captura.Visible = True
                lblMPE_title.Text = "Editar Producto"
                pnlCaptura.Width = 650
                pnlCaptura.Height = 420
                BTNP16.Visible = True
                BTNP16.Text = lstAcciones.Find(Function(p) p.CveTipo = 1 And p.CveAccion = 6).NomAccion
    ...
    Sub mpe_fill()
        Try
            Select Case mpe_op.Text
                Case "action_edit"
                    Dim ObjM As Clientes_ProductosModel = New Clientes_Productos().FindById(regPId.Text, CInt(mpe_regId.Text))
                    DependenciasM.Text = ObjM.DependenciasM
                    ...
                    lstControles.FindAll(Function(p) New ArrayList(excepciones.Split(",")).IndexOf(p.CveMSC.ToString) < 0) _
                        .ForEach(Sub(c)
                                 ...
                                 End Sub)
                    DDLCategoriaE.Enabled = If(DependenciasM.Text = "S", False, True)
            End Select
